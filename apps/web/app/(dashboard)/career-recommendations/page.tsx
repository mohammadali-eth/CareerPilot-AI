...
export default function CareerRecommendationsPage() {
  ...
  const ErrorState = () => (
    <div className="p-12 border border-destructive/20 bg-destructive/5 rounded-2xl flex flex-col items-center justify-center text-center space-y-4 max-w-2xl mx-auto shadow-sm my-12">
  ...
  const EmptyState = () => (
    <div className="p-16 border border-border bg-card/40 backdrop-blur-md rounded-2xl flex flex-col items-center justify-center text-center space-y-6 max-w-3xl mx-auto shadow-glass my-8">
      ...
      <button
        ...
        className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white rounded-xl text-sm font-bold shadow-md shadow-indigo-500/10 transition-all disabled:opacity-50"
      >
  ...
  const MarketDemandCard = ({ demand }: { demand: any }) => {
    ...
    if (demand.demand_score < 60) scoreColor = "text-rose-500";
    else if (demand.demand_score < 80) scoreColor = "text-amber-500";
              ...
              return (
                ...
                >
                  ...
                  <td className="p-4 text-right space-x-2">
                    ...
                    <button
                      onClick={() => openDetails(match)}
                      className="px-2 py-1 rounded text-[11px] font-semibold bg-foreground text-background hover:bg-foreground/90"
                    >
                      Details
                    </button>
                  </td>
      ...
      <div className="fixed inset-0 z-50 flex justify-end bg-background/80 backdrop-blur-sm animate-fade-in">
        ...
        <div className="relative w-full max-w-xl h-full border-l border-border bg-card/95 backdrop-blur-md shadow-2xl p-6 flex flex-col justify-between overflow-y-auto">
                ...
                <div className="p-3 border border-border/80 bg-muted/5 rounded-xl space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {details.required_skills?.map((skill) => (
                      <span
                        key={skill}
                        className="px-2 py-0.5 bg-emerald-500/10 border border-emerald-500/15 rounded text-[10px] text-emerald-400 font-semibold"
                      >
                        {skill}
                      </span>
                    ))}
                  ...
                  <div className="flex flex-wrap gap-1">
                    ...
                    details.missing_skills.length > 0 ? (
                      details.missing_skills.map((skill) => (
                        <span
                          key={skill}
                          className="px-2 py-0.5 bg-amber-500/10 border border-amber-500/15 rounded text-[10px] text-amber-400 font-semibold"
                        >
                          {skill}
                        </span>
                      ))
          ...
          <div className="mt-8 border-t border-border pt-4 flex gap-3">
            ...
            <button
              onClick={() => setIsDrawerOpen(false)}
              className="flex-1 py-2 px-4 bg-foreground hover:bg-foreground/90 text-background text-xs font-semibold rounded-lg transition-all"
            >
              Close Panel
            </button>
          </div>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm animate-fade-in">
        <div className="w-full max-w-4xl border border-border bg-card/95 backdrop-blur-md rounded-2xl shadow-premium p-6 overflow-hidden flex flex-col justify-between max-h-[90vh]">
                      ...
                      c.details.missing_skills.length > 0 ? (
                        <div className="flex flex-wrap gap-1 max-w-[200px]">
                          {c.details.missing_skills.slice(0, 3).map((s) => (
                            <span
                              key={s}
                              className="px-1.5 py-0.5 bg-amber-500/10 border border-amber-500/15 text-[9px] rounded text-amber-400 font-semibold"
                            >
                              {s}
                            </span>
                          ))}
          ...
          <div className="mt-6 border-t border-border pt-4 flex justify-between items-center">
            ...
            <button
              onClick={() => setIsComparisonOpen(false)}
              className="py-2 px-6 bg-foreground hover:bg-foreground/90 text-background text-xs font-semibold rounded-lg transition-all"
            >
              Close Board
            </button>
          </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border pb-6">
        ...
        <div className="flex gap-2.5 self-start">
          ...
          <button
            onClick={handleGenerate}
            disabled={generateMutation.isPending}
            className="inline-flex items-center gap-1.5 px-4 py-2 bg-foreground hover:bg-foreground/90 text-background rounded-lg text-xs font-bold transition-all disabled:opacity-50"
          >
            {generateMutation.isPending ? (
              <>
          ...
          {activeRec && (
            <button
              ...
              className="p-2 border border-border hover:border-rose-500/30 hover:bg-rose-500/5 text-muted-foreground hover:text-rose-400 rounded-lg transition-colors"
            >
            ...
            <div className="fixed bottom-6 right-6 z-40 bg-card border border-border rounded-xl shadow-premium px-4 py-3 flex items-center gap-4 animate-fade-in-up">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-3 py-1.5 bg-primary text-primary-foreground font-semibold rounded-lg text-xs hover:bg-primary/95"
                >
                  Compare Now
                </button>
                <button
