...
export default function MentorPage() {
  ...
  return (
    <div className="flex flex-col gap-6 min-h-screen text-slate-100 max-w-7xl mx-auto">
      {/* 1. Header Banner */}
        ...
        <div className="flex items-center gap-3">
          {view === "chat" && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg bg-slate-800 border border-slate-700 hover:bg-slate-700 transition-colors"
            >
          ...
          <button
            ...
            disabled={submitting}
            className="flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white shadow-lg shadow-indigo-500/20 transition-all disabled:opacity-50"
          >
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
          ...
          <div className="lg:col-span-1 flex flex-col gap-4 p-4 rounded-xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-md">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 text-sm bg-slate-950 border border-slate-800 rounded-lg text-slate-200 placeholder-slate-500 focus:outline-none focus:border-indigo-500/50"
              />
            </div>
            ...
            <div className="flex flex-col gap-2 max-h-[400px] overflow-y-auto pr-1">
              {filteredSessions.length === 0 ? (
                <div className="text-center py-8 text-xs text-slate-500">No sessions found.</div>
              ) : (
                filteredSessions.map((session) => {
                  ...
                  return (
                    <div
                      ...
                      className={`group relative p-3 rounded-lg border text-left cursor-pointer transition-all duration-150 ${
                        isActive
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            ...
                            className={`p-1 rounded hover:bg-slate-800 ${session.pinned ? "text-indigo-400" : "text-slate-500"}`}
                          >
                          ...
                          <button
                            ...
                            className={`p-1 rounded hover:bg-slate-800 ${session.archived ? "text-indigo-400" : "text-slate-500"}`}
                          >
                      ...
                      {session.summary && (
                        <p className="text-xs text-slate-500 truncate mt-1">{session.summary}</p>
                      )}
          ...
          <div className="lg:col-span-3">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {dashboardData.insights.most_discussed_topics.map((t, idx) => (
                            <span key={idx} className="px-2.5 py-1 text-xs rounded-lg bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 font-medium">
                              {t}
                            </span>
                          ))}
                        </div>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {dashboardData.insights.top_weaknesses.map((w, idx) => (
                            <span key={idx} className="px-2.5 py-1 text-xs rounded-lg bg-amber-500/10 border border-amber-500/20 text-amber-300 font-medium">
                              {w}
                            </span>
                          ))}
                        </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {dashboardData?.recommended_actions && dashboardData.recommended_actions.length > 0 ? (
                      dashboardData.recommended_actions.map((act, idx) => (
                        <div key={idx} className="p-4 rounded-lg bg-slate-950 border border-slate-800 flex flex-col justify-between gap-3">
                          ...
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 text-[10px] font-bold text-indigo-400 hover:text-indigo-300 transition-colors uppercase mt-2"
                          >
                            Explore Action
                            <ChevronRight className="h-3 w-3" />
                          </button>
                      ...
                      <div className="col-span-3 text-center py-6 text-slate-500 text-xs">No recommended actions generated.</div>
                    )}
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {SUGGESTED_PROMPTS.map((promptText, idx) => (
                      <button
                        ...
                        className="p-4 rounded-lg bg-slate-900 border border-slate-800 hover:border-indigo-500/30 text-left text-xs text-slate-300 hover:text-indigo-200 transition-all duration-150 flex items-center justify-between group"
                      >
            ...
            {view === "chat" && (
              <div className="flex flex-col h-[650px] rounded-xl bg-slate-900/40 border border-slate-800/80 backdrop-blur-md overflow-hidden">
                          ...
                          <div className="flex flex-col gap-2">
                            <div className={`p-4 rounded-2xl text-sm leading-relaxed whitespace-pre-line ${
                              ...
                              {isBot && explanationContent && (
                                <div className="mt-4 pt-4 border-t border-indigo-500/10 bg-indigo-500/5 -mx-4 -mb-4 p-4 rounded-b-2xl border-t border-slate-800/80">
                ...
                <form onSubmit={handleSendMessage} className="p-3 bg-slate-950 border-t border-slate-800 flex gap-2">
                  <input
                    ...
                    disabled={submitting}
                    className="flex-1 bg-slate-900 border border-slate-800 rounded-lg px-4 py-2.5 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:border-indigo-500/50"
                  />
                  <button
                    type="submit"
                    disabled={!inputText.trim() || submitting}
                    className="px-4 py-2.5 rounded-lg bg-indigo-600 text-white font-semibold text-sm hover:bg-indigo-500 transition-colors disabled:opacity-50"
                  >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
          <div className="w-full max-w-md p-6 rounded-xl bg-slate-900 border border-slate-800 flex flex-col gap-4 shadow-2xl">
            ...
            <input
              ...
              className="w-full px-4 py-2 text-sm bg-slate-950 border border-slate-800 rounded-lg text-slate-200 focus:outline-none focus:border-indigo-500/50"
            />
            <div className="flex justify-end gap-2">
              <button
                onClick={() => setShowRenameModal(false)}
                className="px-4 py-2 text-xs font-semibold rounded bg-slate-800 text-slate-400 hover:bg-slate-700"
              >
                Cancel
              </button>
              <button
                onClick={submitRename}
                className="px-4 py-2 text-xs font-semibold rounded bg-indigo-600 text-white hover:bg-indigo-500"
              >
                Save
              </button>
            </div>
          ...
          <div className="w-full max-w-md p-6 rounded-xl bg-slate-900 border border-slate-800 flex flex-col gap-4 shadow-2xl">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                className="px-4 py-2 text-xs font-semibold rounded bg-rose-600 text-white hover:bg-rose-500"
              >
                Delete
              </button>
            </div>
                ...
                <div className="grid grid-cols-3 gap-2 py-2">
                  {(["markdown", "pdf", "text"] as const).map((type) => (
                    <button
                      ...
                      className={`p-3 rounded-lg border text-xs font-semibold uppercase ${
                        exportType === type
