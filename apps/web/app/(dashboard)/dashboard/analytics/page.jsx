...
import { TrendingUp, Minus, FileText, Activity, MessageSquare, Award, AlertTriangle, RefreshCw, ChevronRight, CheckCircle2, XCircle, Clock, Sparkles } from "lucide-react";
...
export default function AnalyticsDashboardPage() {
    ...
    const [focusedMetric, setFocusedMetric] = useState("readiness");
    ...
    return (<div className="space-y-8 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 border-b border-border pb-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="inline-flex bg-muted/60 p-1 rounded-lg border border-border">
            {["7d", "30d", "90d", "all"].map((range) => (<button key={range} onClick={() => setDateRange(range)} className={`px-3 py-1 text-xs font-semibold rounded-md transition-all ${dateRange === range
          ...
          <button onClick={handleGenerateReport} disabled={generatingReport} className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 text-xs font-bold transition-all shadow-sm shadow-indigo-600/10 disabled:opacity-50">
            {generatingReport ? (<RefreshCw className="h-3.5 w-3.5 animate-spin"/>) : (<Sparkles className="h-3.5 w-3.5"/>)}
            Run Audit Report
          </button>
        </div>
      </div>

      {successMessage && (<div className="p-4 border border-emerald-500/20 bg-emerald-500/5 text-emerald-600 rounded-xl text-xs font-semibold flex items-center gap-2 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div onClick={() => setFocusedMetric("readiness")} className={`glass-panel p-6 rounded-2xl border transition-all duration-300 cursor-pointer relative overflow-hidden flex flex-col justify-between ${focusedMetric === "readiness"
          ...
          <div className="flex items-center gap-4 py-3">
            <div className="relative h-16 w-16 shrink-0 flex items-center justify-center">
              <svg className="absolute inset-0 transform -rotate-90" viewBox="0 0 36 36">
                ...
                <path className="text-indigo-600 transition-all duration-1000 ease-out" strokeWidth="3.5" strokeDasharray={`${latest_metrics.career_readiness_score}, 100`} strokeLinecap="round" stroke="currentColor" fill="none" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"/>
              </svg>
        ...
        {/* Growth Index Score */}
        <div onClick={() => setFocusedMetric("growth")} className={`glass-panel p-6 rounded-2xl border transition-all duration-300 cursor-pointer relative overflow-hidden flex flex-col justify-between ${focusedMetric === "growth"
        ...
        {/* Resume & ATS Score */}
        <div onClick={() => setFocusedMetric("resume")} className={`glass-panel p-6 rounded-2xl border transition-all duration-300 cursor-pointer relative overflow-hidden flex flex-col justify-between ${focusedMetric === "resume"
          ...
          <div className="py-3">
            <div className="flex items-baseline gap-2">
              ...
              <span className={`text-[10px] px-2 py-0.5 font-bold rounded-md border capitalize ${(latest_metrics.resume_score + latest_metrics.ats_score) / 2 >= 75
        ...
        <div onClick={() => setFocusedMetric("readiness_actions")} className={`glass-panel p-6 rounded-2xl border transition-all duration-300 cursor-pointer relative overflow-hidden flex flex-col justify-between ${focusedMetric === "readiness_actions"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 glass-panel p-6 rounded-2xl border border-border space-y-4">
          ...
          <div className="h-64 relative w-full pt-4">
            {activeSnapshots.length < 2 ? (<div className="h-full w-full flex flex-col items-center justify-center text-center space-y-2">
              ...
              </div>) : (<TrendChartSVG snapshots={activeSnapshots} focusedMetric={focusedMetric}/>)}
          </div>
        </div>
        ...
        <div className="glass-panel p-6 rounded-2xl border border-border space-y-5">
          ...
          <div className="space-y-4">
            {readiness_breakdown && Object.entries(readiness_breakdown).map(([key, val]) => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
            ...
            <div className="p-4 rounded-xl border border-emerald-500/20 bg-emerald-500/[0.01] space-y-2">
              ...
              <ul className="space-y-1.5">
                {growth_insights.strengths.map((str, idx) => (<li key={idx} className="text-xs text-slate-700 flex items-start gap-1.5">
            ...
            <div className="p-4 rounded-xl border border-amber-500/20 bg-amber-500/[0.01] space-y-2">
              ...
              <ul className="space-y-1.5">
                {growth_insights.weaknesses.map((weak, idx) => (<li key={idx} className="text-xs text-slate-700 flex items-start gap-1.5">
            ...
            <div className="p-4 rounded-xl border border-blue-500/20 bg-blue-500/[0.01] space-y-2">
              ...
              <ul className="space-y-1.5">
                {growth_insights.opportunities.map((opp, idx) => (<li key={idx} className="text-xs text-slate-700 flex items-start gap-1.5">
            ...
            <div className="p-4 rounded-xl border border-rose-500/20 bg-rose-500/[0.01] space-y-2">
              ...
              <ul className="space-y-1.5">
                {growth_insights.risks.map((risk, idx) => (<li key={idx} className="text-xs text-slate-700 flex items-start gap-1.5">
        ...
        <div className="glass-panel p-6 rounded-2xl border border-border space-y-4">
          ...
          <div className="space-y-3 pt-2">
            {growth_insights.next_steps.map((step, idx) => (<div key={idx} className="flex gap-3 items-start bg-muted/30 border border-border/50 p-3 rounded-xl hover:border-indigo-500/20 transition-all">
      ...
      <div className="glass-panel p-6 rounded-2xl border border-border space-y-4">
        <div className="flex items-center justify-between border-b border-border/60 pb-3">
          ...
          <Link href="/dashboard/reports" className="text-xs font-bold text-indigo-600 hover:underline flex items-center gap-0.5">
            Manage Reports <ChevronRight className="h-3 w-3"/>
          </Link>
        </div>

        {recent_reports.length === 0 ? (<div className="text-center py-8 space-y-2">
            ...
            <table className="w-full text-xs text-left text-slate-700">
              ...
              <tbody className="divide-y divide-border">
                {recent_reports.map((report) => (<tr key={report.id} className="hover:bg-muted/30">
                    ...
                    <td className="px-6 py-3.5 text-right">
                      <Link href="/dashboard/reports" className="text-xs font-bold text-indigo-600 hover:underline inline-flex items-center gap-1">
...
// ==========================================
function TrendChartSVG({ snapshots, focusedMetric }) {
    const width = 800;
    const height = 250;
    const paddingX = 50;
    const paddingY = 40;
    // Metric mappings
    const getMetricVal = (s) => {
    ...
    return (<svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full">
      ...
      {[0, 25, 50, 75, 100].map((tick) => {
            ...
            return (<g key={tick} className="opacity-40">
            <line x1={paddingX} y1={y} x2={width - paddingX} y2={y} stroke="#e2e8f0" strokeWidth="1" strokeDasharray="4,4"/>
            <text x={paddingX - 10} y={y + 4} textAnchor="end" className="text-[10px] font-mono fill-slate-400">
              {tick}%
            </text>
          </g>);
        })}
      ...
      {points.map((p, idx) => {
            ...
            if (pointsCount > 5 && idx % Math.ceil(pointsCount / 5) !== 0 && idx !== pointsCount - 1)
                return null;
            ...
            return (<g key={idx}>
            <line x1={p.x} y1={height - paddingY} x2={p.x} y2={height - paddingY + 5} stroke="#cbd5e1" strokeWidth="1.5"/>
            <text x={p.x} y={height - paddingY + 18} textAnchor="middle" className="text-[9px] font-semibold fill-slate-500">
              {formattedDate}
            </text>
          </g>);
        })}
      ...
      {/* Filled Area */}
      <polygon points={areaPoints} fill={`url(#g-${focusedMetric === "growth"
            ? "growth"
      ...
      {/* Connection Line */}
      <polyline points={polylinePoints} fill="none" stroke={focusedMetric === "growth"
            ? "#10b981"
            : focusedMetric === "resume"
                ? "#a855f7"
                : focusedMetric === "readiness_actions"
                    ? "#f59e0b"
                    : "#4f46e5"} strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round"/>

      {/* Node Dots */}
      {points.map((p, idx) => (<g key={idx} className="group">
          <circle cx={p.x} cy={p.y} r="4.5" fill="#ffffff" stroke={focusedMetric === "growth"
                ? "#10b981"
                : focusedMetric === "resume"
                    ? "#a855f7"
                    : focusedMetric === "readiness_actions"
                        ? "#f59e0b"
                        : "#4f46e5"} strokeWidth="3" className="cursor-pointer transition-all hover:scale-155"/>
          {/* Tooltip on hover */}
...
function ErrorState({ message, onRetry }) {
    return (<div className="max-w-md mx-auto p-6 border border-destructive/20 bg-destructive/5 rounded-2xl flex flex-col items-center justify-center text-center space-y-4 mt-12 shadow-sm">
      ...
      <button onClick={onRetry} className="inline-flex items-center gap-2 px-4 py-2 bg-foreground text-background rounded-lg hover:bg-foreground/90 text-xs font-semibold transition-all">
...
function EmptyState({ onGenerate, loading }) {
    return (<div className="max-w-2xl mx-auto p-12 border border-dashed rounded-3xl flex flex-col items-center justify-center text-center space-y-6 mt-12">
      ...
      <button onClick={onGenerate} disabled={loading} className="inline-flex items-center gap-2 px-6 py-2.5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 text-xs font-bold transition-all shadow-md shadow-indigo-600/15">
